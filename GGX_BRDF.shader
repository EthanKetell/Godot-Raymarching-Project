...
uniform vec4 test_albedo:hint_color = vec4(1.0);
uniform vec4 test_specular:hint_color = vec4(1.0);
...
float hard_shadow(vec3 pos, vec3 dir, float time) {
	float dist = shadow_min_dist+surface_distance;
	for(int i = 0; i < max_steps; i++) {
		float df = dist_field(pos+dir*dist, time);
		dist += df*step_mult;
...
vec3 calc_ggx(vec3 view, vec3 normal, vec3 albedo, vec3 specular, vec3 light_dir, vec3 light_color, float time) {
	...
	{ //schlickFresnel
		F = specular + (1.0 - specular) * pow(1.0 - LdotH, 5.0);
	}
	
	vec3 ggx_term = D*G*F/(4.0/*NdotL*/*NdotV);
	
	return ggx_term + NdotL*light_color*albedo.rgb/3.14159;
}
...
vec3 raymarch(vec3 origin, vec3 dir, float time) {
	...
	vec3 color = ambient_light_color.rgb * ambient_light_intensity;
	color += calc_ggx(dir, normal, test_albedo.rgb, test_specular.rgb, sunlight_direction, sunlight_color.rgb, time);
	color += calc_ggx(dir, normal, test_albedo.rgb, test_specular.rgb, normalize(ray.xyz-vec3(1.5, 0.0, 0.0)), sunlight_color.rgb, time);
	return color;
}
